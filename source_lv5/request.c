  ...
  if (!strstr(uri, "cgi")) {
    ...
    */
    ...
    if (strstr(uri, "?")) {
      ...
      sprintf(filename, ".%s", uri);
      strcpy(cgiargs, ""); // 이전 QUERY_STRING이 남아있지 않기 위해..
    }
    return DYNAMIC;
  }
}
...
{
  char buf[MAXLINE];
  char bodyLengthToString[MAXLINE];
  int res;
  char *argv[] = {NULL};
  ...
  if (!strcmp(Getenv("REQUEST_METHOD"), "POST")) {
    sprintf(bodyLengthToString, "%d", bodyLength);
    Setenv("CONTENT_LENGTH", bodyLengthToString, 1);
  }
  ...
  if (pid == 0) {
    //Child
    Dup2(serverToClient[0], STDIN_FILENO);
...
void requestServeStatic(int fd, char *filename, int filesize, double arrivalTime)
{
  ...
  srcfd = Open1(filename, O_RDONLY, 0);
