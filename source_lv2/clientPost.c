    ...
    if (strstr(userRequest, " ")) {
      ...
      if (!strcmp(command, "name")) {
        ...
        for (int i = 0; i < repeat; i++) {
          // 0.000000 ~ 20.000000 균등 분포!
          randomTemperature = (float)(rand() % (20 * 1000000)) / 1000000 + (value - 10);
          time(&now);
          strcpy(currentCtime, ctime(&now));
          currentCtime[strlen(currentCtime)-1] = '\0';
          sprintf(msg, "name=%s&time=%s&value=%f", myname, currentCtime, randomTemperature);
          printf("Sending %s ...\n", msg);
      ...
      if (!strcmp(userRequest, "help")) {
        ...
        if (currentCtime[strlen(currentCtime)-1] == '\n')
          currentCtime[strlen(currentCtime)-1] = '\0';
        sprintf(msg, "name=%s&time=%s&value=%f", myname, currentCtime, value);
        printf("Sending %s ...\n", msg);
        clientfd = Open_clientfd(hostname, port);
        clientSend(clientfd, filename, msg);
        clientPrint(clientfd);
